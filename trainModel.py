...
def trainModel(train, y_train=None, valid=None, ds_name="", theta=0.7, epochs=3, input_shape=(256, 256, 3), steps_per_epoch=None, use_nn=True):
    ...
    # Train model
    if y_train:
        history = model.fit(train, y_train, epochs=epochs, validation_data=valid, steps_per_epoch=steps_per_epoch)
    else:
        history = model.fit(train, epochs=epochs, validation_data=valid, steps_per_epoch=steps_per_epoch)
